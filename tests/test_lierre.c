...
void test_lierre_strerror_negative_error(void)
{
    const char *str;

    str = lierre_strerror((lierre_error_t)-1);
    TEST_ASSERT_NOT_NULL(str);
    TEST_ASSERT_EQUAL_STRING("Unknown error", str);
}
...
void test_lierre_rgb_create_insufficient_data_size(void)
{
    lierre_rgb_data_t *rgb;
    uint8_t data[12] = {0};

    rgb = lierre_rgb_create(data, 11, 2, 2);
    TEST_ASSERT_NULL(rgb);
}

void test_lierre_rgb_create_exact_minimum_data_size(void)
{
    lierre_rgb_data_t *rgb;
    uint8_t data[12] = {0};

    rgb = lierre_rgb_create(data, 12, 2, 2);
    TEST_ASSERT_NOT_NULL(rgb);
    lierre_rgb_destroy(rgb);
}
...
int main(void)
{
    ...
    RUN_TEST(test_lierre_strerror_negative_error);
    ...
    RUN_TEST(test_lierre_rgb_create_insufficient_data_size);
    RUN_TEST(test_lierre_rgb_create_exact_minimum_data_size);
