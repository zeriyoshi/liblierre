...
static inline bool add_ecc_and_interleave(uint8_t data[], uint8_t version, uint8_t ecl, uint8_t result[])
{
    const uint8_t *dat;
    poporon_t *pprn;
    ...
    config = poporon_rs_config_create(8, RS_GF256_PRIMITIVE_POLY, 0, 1, block_ecc_len, NULL, NULL);
    if (!config) {
        return false;
    }
    ...
    for (i = 0; i < num_blocks; i++) {
        ...
        poporon_encode(pprn, (uint8_t *)dat, (size_t)dat_len, ecc);
...
{
    uint8_t pad_byte, version, best_mask;
    ...
    if (!add_ecc_and_interleave(qrcode, version, ecl, temp_buffer)) {
        return false;
    }
...
extern bool lierre_writer_get_res(const lierre_writer_param_t *param, lierre_reso_t *res)
{
    ...
    if (param->margin > SIZE_MAX / 2) {
        return false;
    }

    if (param->margin * 2 > SIZE_MAX - qr_size) {
        return false;
    }

    if (param->scale != 0 && (qr_size + param->margin * 2) > SIZE_MAX / param->scale) {
        return false;
    }
                                             ...
                                             const lierre_rgba_t *bg_color)
{
    ...
    if (res.width != 0 && res.height > SIZE_MAX / res.width) {
        lfree(writer->param);
        lfree(writer);
        return NULL;
    }

    if (res.width * res.height > SIZE_MAX / 4) {
        lfree(writer->param);
        lfree(writer);
        return NULL;
    }
